// NarrowPhase.cpp : Defines the entry point for the console application.
//
...
void renderScene(){
    ...
    if(collision){
        ...
        //projection on the plane of the colliding point
        /*glBegin(GL_LINES);
...
void updateScene()
{
        ...
        for(int i=0;i<collection.size();i++)
            ...
            if(collection.at(i).box.min.y < pl.origin.y){
                collection.at(i).setCollision(true);
                collision = true;
            }
            else {
                collection.at(i).setCollision(false);
                collision = false;
            }
        ...
        if(collision){
            ...
            for( int i=0; i < colliding_points.size(); i++)
                ...
                for( int i=0; i < colliding_points.size(); i++){
                    mean_coll_point.x += colliding_points.at(i).x;
                    mean_coll_point.y += colliding_points.at(i).y;
                    mean_coll_point.z += colliding_points.at(i).z;
                }

                mean_coll_point/=colliding_points.size();
                std::cout << "resulting (AVGd) vertex colliding: " << mean_coll_point << std::endl;
...
int main(int argc, char *argv[]){
    ...
    windowId = glutCreateWindow("IET Physics Lab 4: Rigid Body Collision (NP)");
